...
export const HeroFilm = ({ imageUrl, title, year, janre, time }) => {
  const movieSrc = getFilmImg(imageUrl);
  return (
    <section className="movie-hero">
      <img src={movieSrc} className="movie-hero__img" loading="lazy" />
      <div className="movie-info">
        ...
        <div className="movie-info__description body-text">
          ...
          {janre ? (
            <div>
              <span>Жанр</span>
              <span className="text-nowrap">{janre}</span>
            </div>
          ) : null}
          {time ? (
            <div>
